...
// setRouteRuleLevel records a route-rule target's first-registered entry. Building the scope here
// (rather than accepting one from the caller) means a caller can never populate a route-rule entry
// under the wrong kind of scope by mistake.
...
func (idx *policyIndex[T]) Lookup(
	...
	return idx.listenerOrGateway(gatewayNN, listenerName), false
}
...
// listenerOrGateway resolves just the listener/gateway levels, for Lookup/resolveEntry's own
// fallback step when no more specific route-rule/route entry applies.
func (idx *policyIndex[T]) listenerOrGateway(gatewayNN types.NamespacedName, listenerName *gwapiv1.SectionName) T {
	var zero T
	if idx == nil {
		return zero
	}
