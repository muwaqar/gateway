...
func TestCTPClusterSettingsIndex(t *testing.T) {
	...
	ctps := []*egv1a1.ClientTrafficPolicy{
		...
		// The oldest-accepted policy claims its slot unconditionally, even without HTTP1 set,
		// blocking a younger conflicting policy targeting the same slot from being counted.
		{
		...
		// Same "oldest wins" semantics at ListenerSet-wide level - unlike Gateway-wide, this level
		// is still tracked, so the precedence gap still applies here.
		{
